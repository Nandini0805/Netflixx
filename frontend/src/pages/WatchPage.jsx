...
const WatchPage = () => {
  ...
  useEffect(() => {
    const getSimilarContent = async () => {
        try {
            console.log('Fetching similar content for:', contentType, id); // Debug log
            const res = await axios.get(`/api/v1/${contentType}/${id}/similar`);
            console.log('Response:', res.data); // Debug log
            if (res.data.success && res.data.similar) {
        ...
        <div className='mx-auto container px-4 py-8 h-full'>
            ...
            {similarContent && similarContent.length > 0 ? (
                <div className='mt-12 max-w-5xl mx-auto relative'>
                    <h3 className='text-3xl font-bold mb-4'>Similar {contentType === 'movie' ? 'Movies' : 'TV Shows'}</h3>
